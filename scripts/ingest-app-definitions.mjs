...
for (const [slug, name, subscription, envKey] of [["anthropic", "Claude", true, "ANTHROPIC_API_KEY"], ["openai", "OpenAI", true, "OPENAI_API_KEY"], ["openrouter", "OpenRouter", false, "OPENROUTER_API_KEY"], ["ollama", "Ollama Cloud", false, "OLLAMA_API_KEY"], ["xai", "Grok", true, "XAI_API_KEY"]]) {
 let app=apps.find(a=>a.slug===slug);
 if(!app){app={schemaVersion:1,slug,name,description:`Connect ${name} accounts for your agents.`,categories:["ai"],branding:brandingFor(slug),urlPatterns:[{"openai":"https://api.openai.com/*","openrouter":"https://openrouter.ai/api/*","ollama":"https://ollama.com/api/*","xai":"https://api.x.ai/*"}[slug]],methods:[]};apps.push(app);}
